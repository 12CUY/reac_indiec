...
const Dashboard = () => {
  ...
  const data = {
    labels: ['Noche Estrellada', 'Viento del Mar', 'Sueños de Otoño', 'Ecos del Pasado', 'El Sol y la Luna'],
    datasets: [
      {
        data: [12, 19, 5, 8, 3],
        ...
        <div className="flex flex-wrap justify-center gap-4 w-full mt-8">
          {['11-01-2025', '12-02-2025', '15-03-2025'].map((date, index) => (
            <motion.div
              ...
              whileHover={{ scale: 1.05 }}
            >
              <p className="text-sm font-medium text-gray-700">Fecha del evento: {date}</p>
          ...
          {songs.map((song) => (
            ...
            >
              ...
              <div className="absolute bottom-0 w-full bg-black bg-opacity-50 py-2">
                <p className="text-center text-white text-sm font-medium">{song.name}</p>
              </div>
            </motion.div>
          ))}
        </div>
